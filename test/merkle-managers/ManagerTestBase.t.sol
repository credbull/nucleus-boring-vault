...
library Roles {
    ...
    function toUint8(Role role) internal pure returns (uint8) {
        return uint8(role);
    }

    function admin() internal returns (uint8) {
        return toUint8(Role.ADMIN);
    }

    function manager() internal returns (uint8) {
        return toUint8(Role.MANAGER);
    }

    function managerInternal() internal returns (uint8) {
        return toUint8(Role.MANAGER_INTERNAL);
    }

    function strategist() internal returns (uint8) {
        return toUint8(Role.STRATEGIST);
    }
}
...
abstract contract ManagerTestBase is Test, MainnetAddresses {
    ...
    function _generateMerkleTree(ManageLeaf[] memory manageLeafs) internal view returns (bytes32[][] memory tree) {
        ...
        // Check for a single-leaf case and directly return
        if (leafsLength == 1) {
            tree = new bytes32[][](1);
            tree[0] = leafs[0];
            return tree;
